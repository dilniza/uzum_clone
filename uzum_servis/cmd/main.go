...
import (
	...
	"github.com/saidamir98/udevs_pkg/logger" //revise
)
...
func main() {
	...
	lis, err := net.Listen("tcp", cfg.CategoryGRPCPort)
	if err != nil {
		log.Panic("net.Listen", logger.Error(err))
	}

	log.Info("GRPC: Server being started...", logger.String("port", cfg.CategoryGRPCPort))
